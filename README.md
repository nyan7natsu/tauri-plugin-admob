...
```toml
[dependencies]
tauri-plugin-google-admob = { git = "https://github.com/nyan7natsu/tauri-plugin-google-admob" }
```

またはコマンドで:

```sh
cargo add tauri-plugin-google-admob --git https://github.com/nyan7natsu/tauri-plugin-google-admob
```
...
```sh
pnpm add github:nyan7natsu/tauri-plugin-google-admob
```

npm / yarn の場合は許可設定は不要です:

```sh
npm install github:nyan7natsu/tauri-plugin-google-admob
```
...
```sh
# アプリリポジトリのルートで
git submodule add https://github.com/nyan7natsu/tauri-plugin-google-admob plugins/tauri-plugin-google-admob
```
...
[dependencies]
tauri-plugin-google-admob = { path = "../plugins/tauri-plugin-google-admob" }
```
...
```
$(SRCROOT)/../../../plugins/tauri-plugin-google-admob/ios/.build/artifacts/...
```
...
git commit -m "chore: update tauri-plugin-google-admob"
```
...
`$(SRCROOT)/../../../plugins/tauri-plugin-google-admob` のように相対パスで書けます:
